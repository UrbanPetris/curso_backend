...
const deleteCarritoById = async (req, res) => {
  try {
    const carrito = await findCarrito(req.params.id);
    if (carrito) {
...
const getProductsInCartById = async (req, res) => {
  try {
    const carrito = await findCarrito(req.params.id);
    if (carrito) {
      //acá habría que eventualmente devolver detalles de los productos
      res.status(200).json({ cartProducts: carrito.productos });
    } else {
      res.status(404).json({ error: "Carrito no encontrado" });
    }
  } catch (err) {
